...
//   sn-109-taomarketcap-subnet-api  GET https://api.taomarketcap.com/public/v1/subnets/109/
//     -> TaoMarketCap subnet snapshot JSON (netuid 109)
// Source-repo / archives surfaces are HEAD/html probes, not GET JSON
// call_subnet_surface territory for this Path B verify.
...
const registry = JSON.parse(
  readFileSync(
    fileURLToPath(
      new URL("../registry/subnets/academia.json", import.meta.url),
    ),
    "utf8",
  ),
);
...
const BODY = {
  ...
  netuid: 109,
  is_active: true,
  latest_snapshot: { id: "8667000-109", netuid: 109 },
};
...
describe("SN109 Academia call_subnet_surface verification (#7120)", () => {
  ...
  test("callSubnetSurface returns the TaoMarketCap JSON body via GET", async () => {
    ...
    assert.equal(result.body.netuid, 109);
    assert.equal(result.body.latest_snapshot.netuid, 109);
  });
  ...
  test("end-to-end through the call_subnet_surface MCP tool, resolved by surface id", async () => {
    ...
    try {
      ...
      assert.equal(result.structuredContent.body.netuid, 109);
    } finally {
