...
export const PUBLIC_ARTIFACTS = [
  ...
  artifact(
    ...
    "Neuron-deregistration activity for one subnet over a 7d or 30d window — the distinct deregistered hotkeys, NeuronDeregistered event count, and average deregistrations per hotkey — served live from the account_events NeuronDeregistered stream at /api/v1/subnets/{netuid}/deregistrations (no static file). Raw deregistration/eviction activity, the exit-side companion to the NeuronRegistered demand in /api/v1/subnets/{netuid}/registrations.",
  ...
  artifact(
    ...
    "Live global Subtensor protocol/governance parameters (#6343) — TaoWeight, StakeThreshold, PendingChildKeyCooldown — queried from the finney RPC at request time with 300s KV cache. Each field is independently null on its own RPC failure.",
    "NetworkParametersArtifact",
  ),
  artifact(
    "randomness",
    "/metagraph/network/randomness.json",
    "Live drand randomness-beacon status (#6730/#6731) — LastStoredRound, OldestStoredRound — queried from the finney RPC at request time with 30s KV cache. A current-state snapshot, not a history feed. Each field is independently null on its own RPC failure.",
    "RandomnessArtifact",
  ),
  artifact(
    "subnet-recycled",
